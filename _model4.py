...
class RHAN(nn.Module):
    def __init__(self, num_users, num_items, embedding_dim, drop_ratio, area_num, cat_num):
        ...
        print('using max pooling to combine')
        print('{} area'.format(area_num))
        self.userembeds = UserEmbeddingLayer(num_users, embedding_dim)
        self.itemembeds = ItemEmbeddingLayer(num_items, embedding_dim)
        self.shan_area = MySHAN(num_users, num_items, embedding_dim, drop_ratio, area_num, self.itemembeds, self.userembeds).cuda()
        self.shan_cat = MySHAN(num_users, num_items, embedding_dim, drop_ratio, cat_num, self.itemembeds, self.userembeds).cuda()

        self.maxpooling = nn.MaxPool1d(kernel_size=2)
        # self.att = AttentionLayer(embedding_dim, drop_ratio)
        with open('{}/cat_dict.txt'.format(dataset), 'r', encoding='UTF-8') as f:
            self.item_cat = eval(f.read())
        with open('{}/area_dict_{}.txt'.format(dataset, area_num), 'r', encoding='UTF-8') as f:
    ...
    def forward(self, user_inputs, record_inputs, item_inputs):
        ...
        for i in range(user_inputs.__len__()):
            ...
            # hybrid = self.shan_area(user, area_input)
            ...
            # 连接区域和类型表示 2 * K
            area_cat = torch.cat((area, cat), dim=0)

            # using max_pooling layer
            area_cat = torch.reshape(torch.transpose(area_cat, 0, 1),
                                           (1, self.embedding_dim, 2))
            hybrid = self.maxpooling(area_cat)

            # # 权重 1 * 2
            # at_wt = self.att(area_cat)
            # # 混合表示 1 * K
            # hybrid = torch.matmul(at_wt, area_cat)
...
class MySHAN(nn.Module):
    def __init__(self, num_users, num_items, embedding_dim, drop_ratio, input_num, item_embeds, user_embeds):
        super(MySHAN, self).__init__()
        self.userembeds = user_embeds
        self.itemembeds = item_embeds

        # init using list
        attention_list = []
        for i in range(input_num):
            attention_list.append(AttentionLayer(2 * embedding_dim, drop_ratio).cuda())
        self.attention_list = nn.Sequential(*attention_list)
